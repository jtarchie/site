...
provides one, building it would be helpful to understand why. Note that this
won't be an exhaustive tutorial on Go's concurrency but more of a hands-on
exploration.

> Note: This an introduction to the concept and implementation. It will not end
> up being a production-quality mutex.

A mutex, at its core, is a lock that ensures exclusive access execution of code.
This helps prevents synchronization errors, ensure data integrity, and can limit
bad actors

Consider the banking dilemma. If two transactions—a withdrawal and a
deposit—coincide, they can interfere because the amount of the account balance.
If we start with an account balance of $100, have a withdrawal ($20) and deposit
($30) occur, we should have $110 at the end of all transactions.
...
cycles while waiting for unlock of another segment of code. The go runtime
appears to [preemptive scheduling](https://github.com/golang/go/issues/10958) to
prevent this tight loops. We want to do our best to not block other goroutines
from running.

When looking at the Go implementation of
[`sync.Mutex`](https://pkg.go.dev/sync#Mutex.Lock), the implementation is
similar for a fast path. There is rather extensive implementation to prevent CPU
